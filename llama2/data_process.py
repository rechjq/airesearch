...
def process_wudao():
    wudao_zh_paths = glob.glob('/root/datasets/WuDaoCorpus2.0_base_200G/*')
    wudao_zh_paths=sorted(wudao_zh_paths)
    print(len(wudao_zh_paths))#很多子文件
    cnt=0
    token=0
    doc_ids=[]
    for per in tqdm(wudao_zh_paths[:20]):#wudao_zh_paths[i:j]手动分片，一片片处理，不然太大一次性处理不完
        with open(per,'r') as f:
            data=json.load(f)
            for text in data:
                text = text['title'] + text['content']
                text_id=tokenizer.encode(text)
                text_id.append(tokenizer.eos_id())
                if len(text_id)>5:
                    doc_ids+=text_id
                #
                # if cnt%10000==0:
    ...
    with open('/root/datasets/wudaocorpus_zh_32.bin','wb') as f:
        f.write(arr.tobytes())
    print(arr.shape)
